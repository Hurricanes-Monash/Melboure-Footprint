...
class MapViewController: UIViewController ,CLLocationManagerDelegate{
    ...
    @IBAction func backToCurrentLocation(sender: AnyObject) {
        ...
        let latitude = locationManager1.location!.coordinate.latitude
        let longitude = locationManager1.location!.coordinate.longitude
...
//    }
    ...
    //Set the first look when users use the map, if there are categories, users will be leadding to
    //the categories. However, users will be leading to thier current location
    func locationManager(manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        if !isInitialized {
            // Here is called only once
            isInitialized = true
            //Check if there are categoies
            let location = locations.last
                    let center = CLLocationCoordinate2D(latitude: location!.coordinate.latitude,
                                                        longitude: location!.coordinate.longitude)
